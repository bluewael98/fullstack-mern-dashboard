...
const Daily = () => {
  const [startDate, setStartDate ] = useState(new Date("2021-02-01")); //initialize startDate with a date
  const [endDate, setEndDate ] = useState(new Date("2021-03-01"));  //ditto but for endDate
  ...
  const [formattedData] = useMemo(() => {
    ...
    const { dailyData } = data; // destructure daily data from the data in the useGetSalesQuery
    ...
    Object.values(dailyData).forEach(({ date, totalSales, totalUnits }) => {
      ...
      if (dateFormatted >= startDate && dateFormatted <= endDate) { // allows to chart the line only between start date and end date
       const splitDate = date.substring(date.indexOf("-") + 1); //removes year from date, allows to edit month/day
